...
- `background-catalog.js`: único propietario del catálogo local de planos del recinto. Incluye Casa Acapulco como fondo inmutable por defecto desde `assets/distribucion/casa-acapulco.png` y usa IndexedDB exclusivamente para imágenes personalizadas y preferencias locales de fondo (selección, visibilidad, opacidad y offset). El plano base se monta en el mismo mundo físico fijo 1448 × 1086 a 32 px/m; la cámara solo transforma la vista. No guarda mesas, sillas, invitados, propuestas ni datos canónicos de la boda; no usa Firebase, Firestore ni Storage. Esta es una excepción explícita y limitada a la regla general de no usar almacenamiento del navegador en Distribución.
