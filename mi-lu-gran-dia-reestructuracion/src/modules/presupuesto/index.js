...
function tagColor(value){const color=String(value||'');return /^#[0-9a-f]{6}$/i.test(color)?color:'#777777'}
function csvCell(value){const raw=String(value??'');const safe=/^[=+\-@\t\r]/.test(raw)?"'"+raw:raw;return '"'+safe.replaceAll('"','""')+'"'}
...
function openPackageChooser(item){
 ...
 prepareBudgetDialog(dialog);
 dialog.showModal();
}
function formField(label,control){return '<label class="budget-form-field"><span>'+label+'</span>'+control+'</label>'}
function prepareBudgetDialog(dialog){
 if(!dialog)return;
 dialog.querySelectorAll('form').forEach(form=>form.setAttribute('autocomplete','off'));
 dialog.querySelectorAll('input,textarea,select').forEach(control=>{
  control.setAttribute('autocomplete','off');
  if(control.matches('input:not([type="number"]):not([type="date"]):not([type="color"]):not([type="checkbox"]):not([type="radio"]),textarea')){
...
function openEditor(kind,payload={}){if(!canEdit())return;const root=document.querySelector('[data-module-view="presupuesto"]'),dialog=root?.querySelector('[data-budget-dialog]');if(!dialog)return;const title=root.querySelector('[data-budget-dialog-title]'),body=root.querySelector('[data-budget-dialog-body]'),footer=root.querySelector('[data-budget-dialog-footer]');let html='';
 if(kind==='item'){const item=payload.item||{},categoryId=payload.categoryId||item.categoryId||state.categories[0]?.id||'';title.textContent=item.id?'Editar gasto':'Nuevo gasto';html='<form autocomplete="off" data-budget-edit-form data-kind="item" data-id="'+esc(item.id||'')+'">'+formField('Gasto','<input name="name" autocomplete="off" autocorrect="off" autocapitalize="sentences" spellcheck="false" required maxlength="140" value="'+esc(item.name||'')+'">')+'<div class="budget-form-grid">'+formField('Categoría','<select name="categoryId">'+state.categories.map(cat=>'<option value="'+esc(cat.id)+'"'+(String(cat.id)===String(categoryId)?' selected':'')+'>'+esc(cat.name||'Categoría')+'</option>').join('')+'</select>')+formField('Estado','<select name="status">'+[['pending','Pendiente'],['quoted','Cotizado'],['partial','Pago parcial'],['paid','Pagado']].map(([v,l])=>'<option value="'+v+'"'+((item.status||'pending')===v?' selected':'')+'>'+l+'</option>').join('')+'</select>')+'</div><div class="budget-form-grid">'+formField('Presupuestado','<input name="planned" type="number" min="0" step="0.01" value="'+n(item.planned)+'">')+formField('Cotizado','<input name="quoted" type="number" min="0" step="0.01" value="'+n(item.quoted)+'">')+'</div><div class="budget-form-grid">'+formField('Pagado','<input name="paid" type="number" min="0" step="0.01" value="'+n(item.paid)+'">')+formField('Garantía','<input name="guarantee" type="number" min="0" step="0.01" value="'+n(item.guarantee)+'">')+'</div><div class="budget-form-grid">'+formField('Proveedor','<input name="provider" autocomplete="off" autocorrect="off" autocapitalize="words" spellcheck="false" maxlength="140" value="'+esc(item.provider||'')+'">')+formField('Fecha de pago','<input name="paymentDate" type="date" value="'+esc(item.paymentDate||'')+'">')+'</div>'+formField('Etiquetas','<div class="budget-tag-picker">'+state.tags.map(tag=>'<label style="--tag:'+esc(tagColor(tag.color))+'"><input type="checkbox" name="tagIds" value="'+esc(tag.id)+'"'+(tagIds(item).includes(String(tag.id))?' checked':'')+'><i></i><span>'+esc(tag.name||'Etiqueta')+'</span></label>').join('')+'</div>')+formField('Notas','<textarea name="notes" autocomplete="off" autocorrect="off" autocapitalize="sentences" spellcheck="false" rows="3" maxlength="700">'+esc(item.notes||'')+'</textarea>')+'</form>'}
 else if(kind==='category'){const cat=payload.category||{};title.textContent=cat.id?'Editar categoría':'Nueva categoría';html='<form autocomplete="off" data-budget-edit-form data-kind="category" data-id="'+esc(cat.id||'')+'">'+formField('Nombre','<input name="name" autocomplete="off" autocorrect="off" autocapitalize="sentences" spellcheck="false" required maxlength="80" value="'+esc(cat.name||'')+'">')+formField('Descripción','<input name="description" autocomplete="off" autocorrect="off" autocapitalize="sentences" spellcheck="false" maxlength="180" value="'+esc(cat.description||'')+'">')+'<div class="budget-form-field"><span>Icono</span>'+categoryIconPicker(cat.icon||'•')+'</div></form>'}
 else if(kind==='tag'){const tag=payload.tag||{};title.textContent=tag.id?'Editar etiqueta':'Nueva etiqueta';html='<form autocomplete="off" data-budget-edit-form data-kind="tag" data-id="'+esc(tag.id||'')+'">'+formField('Nombre','<input name="name" autocomplete="off" autocorrect="off" autocapitalize="sentences" spellcheck="false" required maxlength="50" value="'+esc(tag.name||'')+'">')+formField('Color','<input name="color" type="color" value="'+esc(/^#[0-9a-f]{6}$/i.test(tag.color||'')?tag.color:'#7f8962')+'">')+'</form>'}
 else if(kind==='package'){const pack=payload.package||{},members=new Set(packageItems(pack.id).map(i=>String(i.id)));title.textContent=pack.id?'Editar paquete':'Nuevo paquete';html='<form autocomplete="off" data-budget-edit-form data-kind="package" data-id="'+esc(pack.id||'')+'">'+formField('Nombre del paquete','<input name="name" autocomplete="off" autocorrect="off" autocapitalize="sentences" spellcheck="false" required maxlength="100" placeholder="Ej. Paquete Integral" value="'+esc(pack.name||'')+'">')+'<div class="budget-form-grid">'+formField('Empresa / proveedor','<input name="provider" autocomplete="off" autocorrect="off" autocapitalize="words" spellcheck="false" maxlength="140" value="'+esc(pack.provider||'')+'">')+formField('Forma de cálculo','<select name="pricingMode"><option value="package"'+((pack.pricingMode||'package')==='package'?' selected':'')+'>Precio por todo el paquete</option><option value="items"'+(pack.pricingMode==='items'?' selected':'')+'>Sumar precios individuales</option></select>')+'</div><div class="budget-form-grid">'+formField('Presupuestado del paquete','<input name="planned" type="number" min="0" step="0.01" value="'+n(pack.planned)+'">')+formField('Cotizado / contratado','<input name="quoted" type="number" min="0" step="0.01" value="'+n(pack.quoted)+'">')+'</div><div class="budget-form-grid">'+formField('Pagado','<input name="paid" type="number" min="0" step="0.01" value="'+n(pack.paid)+'">')+formField('Garantía','<input name="guarantee" type="number" min="0" step="0.01" value="'+n(pack.guarantee)+'">')+'</div>'+'<section class="budget-package-concepts"><header><div><span class="budget-package-concepts-icon">☷</span><span><b>Conceptos incluidos</b><small>Selecciona los conceptos que formarán parte de este paquete.</small></span></div><strong data-package-selected-count>'+members.size+' seleccionados</strong></header><div class="budget-package-picker">'+state.items.filter(i=>!i.packageId||String(i.packageId)===String(pack.id)).map(i=>'<label><input type="checkbox" name="itemIds" value="'+esc(i.id)+'"'+(members.has(String(i.id))?' checked':'')+'><span class="budget-package-check"></span><span class="budget-package-concept-copy"><b>'+esc(i.name||'Gasto')+'</b><small>'+esc(categoryName(i.categoryId))+'</small></span><strong>'+money(n(i.quoted)||n(i.planned))+'</strong></label>').join('')+'</div></section>'+formField('Notas','<textarea name="notes" autocomplete="off" autocorrect="off" autocapitalize="sentences" spellcheck="false" rows="3" maxlength="700">'+esc(pack.notes||'')+'</textarea>')+'</form>'} body.innerHTML=html;footer.innerHTML='<button type="button" data-budget-close>Cancelar</button><button class="budget-save" type="submit" form="'+(body.querySelector('form')?.id||'')+'" data-budget-save>Guardar cambios</button>';const form=body.querySelector('form');if(form){form.id='budgetEditorForm';footer.querySelector('[data-budget-save]').setAttribute('form','budgetEditorForm')}if(kind==='package'){const count=()=>{const target=body.querySelector('[data-package-selected-count]');if(target)target.textContent=body.querySelectorAll('input[name="itemIds"]:checked').length+' seleccionados'};body.querySelector('.budget-package-picker')?.addEventListener('change',count)}prepareBudgetDialog(dialog);dialog.showModal();if(!window.matchMedia('(max-width:760px)').matches)form?.querySelector('input,select,textarea')?.focus()}
