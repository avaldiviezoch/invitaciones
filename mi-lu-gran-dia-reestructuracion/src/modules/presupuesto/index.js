...
function openBudgetDialog(dialog,{focusFirst=false}={}){
 ...
 dialog.showModal();
 if(focusFirst&&!window.matchMedia('(max-width:760px)').matches){
...
async function mountPresupuesto(context){const root=document.querySelector('[data-module-view="presupuesto"]');if(!root||!context?.id)return false;const epoch=++mountEpoch;activeContext=context;pendingSave=null;root.innerHTML='<div class="budget-empty">Cargando Presupuesto desde Firebase…</div>';try{const [template,stored]=await Promise.all([fetch(new URL('./index.html?v=8',import.meta.url)).then(r=>{if(!r.ok)throw new Error('No se pudo cargar la interfaz.');return r.text()}),readPlannerStorageKey(context,STORAGE_KEY)]);if(epoch!==mountEpoch||activeContext?.id!==context.id)return false;root.innerHTML=template;state=normalized(stored);search='';bind(root);render();const editable=weddingCapabilities(context.role).canEdit;root.querySelector('[data-budget-state]').textContent=editable?'Datos de la boda activa · edición habilitada':'Datos reales de Firebase · solo lectura';return true}catch(error){if(epoch!==mountEpoch)return false;root.innerHTML='<div class="budget-empty">No se pudo cargar el Presupuesto: '+esc(error?.message||'Error de lectura')+'</div>';return false}}
