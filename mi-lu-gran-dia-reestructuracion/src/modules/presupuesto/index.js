...
function itemMarkup(item){const tags=tagIds(item).map(tagById).filter(Boolean);const balance=itemBalance(item),editable=canEdit();return '<div class="budget-item" data-item-id="'+esc(item.id)+'"'+(editable?' draggable="true" data-budget-draggable-item':'')+'><div class="budget-item-main"><strong>'+esc(item.name||'Gasto')+'</strong><small>'+esc(itemMeta(item))+'</small><div class="budget-item-tags">'+tags.map(t=>'<span class="budget-item-tag" style="--tag:'+esc(t.color||'#777')+'">'+esc(t.name)+'</span>').join('')+'</div></div>'+[['Presup.',item.planned],['Cotizado',item.quoted],['Pagado',item.paid],['Saldo',balance]].map(([l,v])=>'<div class="budget-item-value"><span>'+l+'</span><strong>'+money(v)+'</strong></div>').join('')+'<div class="budget-item-actions">'+(editable?'<button class="budget-item-group" type="button" data-budget-group-item aria-label="Agrupar gasto en un paquete" title="Agrupar">↗</button>':'')+'<button class="budget-item-action" type="button" data-budget-view-item aria-label="'+(editable?'Editar gasto':'Ver gasto')+'">'+(editable?'✎':'＋')+'</button></div></div>'}
